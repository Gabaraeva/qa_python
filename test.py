...
def test_get_books_genre_positive():
    ...
    assert result["Преступление и наказание"] == "Детективы"
    assert result["Война и мир"] == ""

def test_get_books_genre_returns_copy():
    collector = BooksCollector()
    collector.add_new_book("Книга")
    collector.set_book_genre("Книга", "Фантастика")  # Устанавливаем жанр
    
    # Получаем словарь и пытаемся его изменить
    books_genre = collector.get_books_genre()
    books_genre["Книга"] = "Хакерский жанр"
...
def test_get_list_of_favorites_books_positive():
    collector = BooksCollector()
    books = ["Мастер и Маргарита", "Преступление и наказание", "Война и мир"]
    for book in books:
        collector.add_new_book(book)
        collector.add_book_in_favorites(book)
    assert collector.get_list_of_favorites_books() == books

def test_get_empty_favorites_list():
    collector = BooksCollector()
